...
func runVolumeTest(ctx context.Context, db *core.Database, config VolumeTestConfig) (*TestResults, error) {
	...
	db.DeleteCollection(ctx, collectionName)
	...
	if stats != nil {
		results.IndexSizeMB = float64(stats.MemoryUsage) / 1024 / 1024
		printPerf(fmt.Sprintf("Index size: %.2f MB", results.IndexSizeMB))
		printPerf(fmt.Sprintf("Index stats: %d vectors, max layer: %d", stats.VectorCount, stats.MaxLayer))
	}
	...
	// Cleanup
	if err := db.DeleteCollection(ctx, collectionName); err != nil {
		printInfo(fmt.Sprintf("Warning: failed to delete collection: %v", err))
	} else {
...
func runComparativeTests(ctx context.Context, db *core.Database) {
...
func runLargeScaleTests(ctx context.Context, db *core.Database) {
...
func testDistanceMetrics(ctx context.Context, db *core.Database) {
