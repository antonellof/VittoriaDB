#!/usr/bin/env python3
"""
Advanced RAG Engine using VittoriaDB as Vector Database with Datapizza AI

This implements a complete RAG system following datapizza-ai patterns:
1. Document chunking with overlap
2. Embedding generation using datapizza-ai (OpenAI/Ollama)
3. Semantic retrieval with VittoriaDB
4. Context management and source attribution
5. Response generation with citations

Based on datapizza-ai RAG guide: https://docs.datapizza.ai/0.0.2/Guides/RAG/rag/
Using VittoriaDB as vector database instead of Qdrant.
"""
...
# Datapizza AI embeddings (following datapizza-ai RAG guide)
...
class VittoriaRAGEngine:
    ...
    def __init__(self, 
        ...
        """
        Initialize the RAG engine following datapizza-ai patterns.
        ...
        Args:
            ...
            use_ollama: Use Ollama for local embeddings
        """
        ...
        # Initialize Datapizza embedder (following datapizza-ai RAG guide)
        if embedder_config:
            ...
            if use_ollama or os.getenv("EMBEDDER_PROVIDER", "").lower() == "ollama":
                ...
                logger.info("🚀 Using Ollama embeddings (datapizza-ai)")
            else:
                embedder_config = EmbedderConfig(
                    provider="openai",
                ...
                logger.info("🚀 Using OpenAI embeddings (datapizza-ai)")
        ...
        logger.info(f"✅ Datapizza embedder initialized: {self.embedder.config.provider} "
                   f"({self.embedder.config.model_name}, {self.embedding_dim}D)")
        ...
        try:
            ...
            try:
                self.collection = self.db.create_collection(
                    ...
                    # Use VittoriaDB server-side vectorizer for OpenAI, client-side for Ollama
                    vectorizer_config=Configure.Vectors.openai_embeddings(
                        api_key=self.embedder.config.api_key
                    ) if (self.embedder.config.provider == "openai" and not self.embedder.config.base_url) else None
                )
    ...
    async def _store_chunk_with_auto_embedding(self, chunk: DocumentChunk):
        """
        Store chunk using datapizza-ai embeddings.
        Following datapizza-ai RAG guide with VittoriaDB storage.
        """
        # Check if we need client-side or server-side embeddings
        if self.embedder.config.provider == "ollama" or self.embedder.config.base_url:
            # Use datapizza-ai client-side embeddings (for Ollama or custom endpoints)
            chunk_embedding = await self.embedder.embed_text(chunk.content)
            ...
            # Use direct vector insertion with pre-computed embedding
            self.collection.insert(
                id=chunk.id,
                vector=chunk_embedding,
                metadata=chunk_metadata
            )
            logger.debug(f"✅ Stored chunk with client-side datapizza embeddings: {chunk.id}")
        else:
            # Use VittoriaDB server-side vectorizer (for standard OpenAI)
            chunk_metadata = {
                'document_id': chunk.document_id,
                'document_title': chunk.document_title,
                'chunk_index': chunk.chunk_index,
            ...
            self.collection.insert_text(
                id=chunk.id,
                text=chunk.content,
                metadata=chunk_metadata
            )
            logger.debug(f"✅ Stored chunk with server-side vectorizer: {chunk.id}")
    ...
    async def search(self, 
        ...
        try:
            # Generate query embedding using datapizza-ai (following RAG guide)
            embed_start = time.time()
            query_embedding = await self.embedder.embed_text(query)
            embed_time = time.time() - embed_start
            logger.debug(f"Generated query embedding in {embed_time:.3f}s using datapizza-ai")
